      ...
      <main id="main-content" className="mx-auto max-w-[1400px] px-4 py-8 lg:px-10 lg:py-12">
        <div className="mb-8 flex flex-col gap-5 border-b-2 border-foreground pb-7 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground">Inspect intent, enforce a budget, and keep an auditable decision trail before an autonomous agent can touch your wallet.</p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <div className="mt-8 grid gap-3 lg:grid-cols-[1.35fr_0.65fr]">
              ...
              <section className="min-w-0 border-2 border-foreground bg-foreground p-5 text-background">
                ...
                <p className="mt-2 text-xs text-background/65">Average risk score across recorded decisions.</p>
            ...
            <section className="mt-8"><div className="mb-4 flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Local workspace</p><h2 className="mt-2 text-2xl font-bold tracking-tight">Your agents</h2></div><button onClick={() => setTab("agents")} className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground">Manage agents <ArrowRight className="ml-1 inline" size={12} /></button></div><div className="grid gap-3 md:grid-cols-2">{agents.slice(0, 2).map((agent) => <AgentCard key={agent.id} agent={agent} onSelect={() => { setActiveAgentId(agent.id); setTab("guard") }} />)}{!agents.length && <div className="md:col-span-2"><EmptyState title="No agents yet" body="Create an identity, define its budget, then optionally register it on 0G." action="Create agent" onClick={() => { setShowAgentForm(true); setTab("agents") }} /></div>}</div></section>
          </motion.div>
        )}
        ...
        {tab === "agents" && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <div className="mb-6 flex flex-col gap-3 border-2 border-foreground bg-foreground p-5 text-background sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-bold tracking-tight">Agent identities</h2><p className="mt-1 max-w-xl text-xs text-background/60">Create an identity locally, then anchor its spending limits in the deployed 0G registry with your wallet.</p></div><button onClick={() => setShowAgentForm((value) => !value)} className="flex items-center justify-center gap-2 bg-background px-4 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-foreground"><Plus size={14} /> Create agent</button></div>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
              <section className="border-2 border-foreground bg-background p-5">
                <div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Action request</p><h2 className="mt-1 text-2xl font-bold tracking-tight">What does the agent want to do?</h2></div><Sparkles size={20} className="text-[#ea580c]" /></div>
                <div className="space-y-4">
                  ...
                  <label className="block"><span className="field-label">Agent instruction / intent</span><textarea value={guardForm.instruction} onChange={(event) => setGuardForm((current) => ({ ...current, instruction: event.target.value }))} className="field-input min-h-24 resize-y" /></label>
              ...
              <GuardResultPanel result={guardResult} onApprove={approveReview} onExecute={executeTransaction} />
            </div>
          </motion.div>
        )}
        ...
        {tab === "policies" && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <div className="mt-6 border-2 border-foreground bg-background p-5"><div className="flex items-start gap-3"><CircleAlert size={17} className="mt-0.5 text-[#a16207]" /><div><h2 className="text-sm font-bold uppercase tracking-[0.12em]">Policy evaluation order</h2><p className="mt-2 max-w-2xl text-xs leading-relaxed text-muted-foreground">AgentShield evaluates the destination, amount, protocol allowlist, and instruction intent in sequence. A blocked result never reaches the wallet. Review results require a human approval before execution.</p></div></div></div>
          </motion.div>
        )}
        ...
        {tab === "activity" && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <section className="border-2 border-foreground bg-background"><div className="flex flex-col gap-3 border-b-2 border-foreground px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold tracking-tight">Every decision, inspectable</h2><p className="mt-1 text-xs text-muted-foreground">Guard decisions and confirmed transaction hashes stay in this browser workspace.</p></div><ClearWorkspaceControl onConfirm={clearWorkspace} /></div><div className="divide-y divide-border">{activity.map((item) => <ActivityRow key={item.id} item={item} detailed />)}{!activity.length && <EmptyState title="Your ledger is empty" body={agents.length ? "Run a security check to record a decision." : "Create an agent before running a security check."} action={agents.length ? "Run guard" : "Create agent"} onClick={() => agents.length ? setTab("guard") : (setShowAgentForm(true), setTab("agents"))} />}</div></section>
          </motion.div>
        )}
      </main>
...
function GuardResultPanel({ result, onApprove, onExecute }: { result: GuardResult | null; onApprove: () => void; onExecute: () => void }) {
  if (!result) return <section className="flex min-h-[560px] flex-col items-center justify-center border-2 border-dashed border-border bg-background p-8 text-center"><div className="grid h-14 w-14 place-items-center border-2 border-foreground"><ShieldCheck size={25} /></div><h2 className="mt-5 text-xl font-bold tracking-tight">Guard result appears here</h2><p className="mt-2 max-w-sm text-xs leading-relaxed text-muted-foreground">Submit an agent action to see its risk score, policy reasons, and the exact next step.</p><div className="mt-7 flex items-center gap-3 text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><span className="h-1.5 w-1.5 bg-[#ea580c]" /> intent <ArrowRight size={12} /> policy <ArrowRight size={12} /> decision</div></section>
  const blocked = result.decision === "BLOCKED"
  return <section className={`border-2 p-5 ${blocked ? "border-[#b91c1c] bg-[#fff7f7]" : result.decision === "REVIEW" ? "border-[#a16207] bg-[#fffdf5]" : "border-[#3f6212] bg-[#f8fff2]"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Security decision</p><h2 className="mt-2 text-3xl font-bold tracking-tight">{blocked ? "Action blocked" : result.decision === "REVIEW" ? "Human approval required" : "Policy passed"}</h2></div><DecisionBadge decision={result.decision} /></div><div className="mt-8 flex items-end gap-4 border-y border-current/20 py-5"><p className="text-7xl font-bold leading-none" style={{ fontVariantNumeric: "tabular-nums" }}>{result.risk}</p><div className="pb-1"><p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Risk score / 100</p><p className="mt-1 text-xs text-muted-foreground">{blocked ? "Do not sign this request." : result.decision === "REVIEW" ? "A person must approve this request." : "Safe to execute within policy."}</p></div></div><div className="mt-6"><p className="field-label">Why this decision</p><ul className="mt-3 space-y-3">{result.reasons.map((reason) => <li key={reason} className="flex items-start gap-2 text-xs"><span className={`mt-1 h-1.5 w-1.5 shrink-0 ${blocked ? "bg-[#b91c1c]" : result.decision === "REVIEW" ? "bg-[#a16207]" : "bg-[#3f6212]"}`} />{reason}</li>)}</ul></div><div className="mt-8 grid gap-2 sm:grid-cols-2">{result.decision === "REVIEW" && <button onClick={onApprove} className="flex items-center justify-center gap-2 border-2 border-foreground px-3 py-3 text-[10px] font-bold uppercase tracking-[0.13em] hover:bg-foreground hover:text-background"><Check size={14} /> Approve once</button>}{!blocked && <button onClick={onExecute} className="flex items-center justify-center gap-2 bg-foreground px-3 py-3 text-[10px] font-bold uppercase tracking-[0.13em] text-background hover:-translate-y-0.5"><Zap size={14} /> Execute on 0G</button>}{blocked && <div className="flex items-center justify-center gap-2 border-2 border-[#b91c1c] px-3 py-3 text-[10px] font-bold uppercase tracking-[0.13em] text-[#b91c1c]"><X size={14} /> Wallet call suppressed</div>}</div></section>
}

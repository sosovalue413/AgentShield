        ...
        {tab === "overview" && (
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease }}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Stat label="Budget available" value={formatPolicyNumber(protectedFunds)} detail="Policy units across agents" />
            </div>
            ...
            <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
              <section className="border-2 border-foreground bg-background p-5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="field-label">Amount</span><input inputMode="decimal" value={guardForm.amount} onChange={(event) => setGuardForm((current) => ({ ...current, amount: event.target.value }))} className="field-input" placeholder="0.00" /></label><label className="block"><span className="field-label">Protocol address</span><input value={guardForm.protocol} onChange={(event) => setGuardForm((current) => ({ ...current, protocol: event.target.value }))} className="field-input" placeholder="0x…" spellCheck={false} /></label></div>
...
function AgentCard({ agent, onSelect, onRegister, expanded = false, registering = false, registered = false }: { agent: Agent; onSelect: () => void; onRegister?: () => void; expanded?: boolean; registering?: boolean; registered?: boolean }) {
  ...
  const syncNeeded = registered && !agent.onchainPolicySynced
  const stateLabel = paused ? "PAUSED" : syncNeeded ? "SYNC NEEDED" : registered ? "SYNCED" : agent.status
  const stateTone = paused || syncNeeded ? "border-[#a16207] text-[#a16207]" : "border-[#3f6212] text-[#3f6212]"
  return <article className="border-2 border-foreground bg-background p-5 transition-transform hover:-translate-y-0.5"><div className="flex items-start justify-between gap-4"><div className="flex items-start gap-3"><div className="grid h-10 w-10 place-items-center border-2 border-foreground bg-foreground text-background"><Fingerprint size={18} /></div><div><h3 className="text-sm font-bold uppercase tracking-[0.1em]">{agent.name}</h3><p className="mt-1 text-xs text-muted-foreground">{agent.purpose}</p></div></div><span className={`flex items-center gap-2 border px-2 py-1 text-[9px] font-bold uppercase tracking-[0.12em] ${stateTone}`}><span className={`h-1.5 w-1.5 ${paused || syncNeeded ? "bg-[#a16207]" : "bg-[#3f6212]"}`} /> {stateLabel}</span></div><div className="mt-5 grid grid-cols-3 gap-3 border-t border-border pt-4"><div><p className="field-label">Trust</p><p className="mt-1 text-lg font-bold">{agent.trust}<span className="text-xs text-muted-foreground">/100</span></p></div><div><p className="field-label">Max tx</p><p className="mt-1 text-lg font-bold">{formatPolicyNumber(agent.maxTransaction)}</p></div><div><p className="field-label">Available</p><p className="mt-1 text-lg font-bold">{formatPolicyNumber(remaining)}</p></div></div>{expanded && <div className="mt-4 border-t border-border pt-4"><div className="flex items-center justify-between text-[10px] uppercase tracking-[0.13em] text-muted-foreground"><span>Wallet</span><span>{agent.wallet}</span></div><div className="mt-2 flex items-center justify-between gap-4 text-[10px] uppercase tracking-[0.13em] text-muted-foreground"><span>Protocols</span><span className="truncate">{agent.protocols.length ? agent.protocols.map(shortAddress).join(" · ") : "None"}</span></div></div>}<div className={`mt-5 grid gap-2 ${onRegister ? "sm:grid-cols-2" : ""}`}><button onClick={onSelect} className="flex w-full items-center justify-center gap-2 border-2 border-foreground px-3 py-2 text-[10px] font-bold uppercase tracking-[0.13em] transition-colors hover:bg-foreground hover:text-background">Evaluate action <ArrowRight size={12} /></button>{onRegister && <button onClick={onRegister} disabled={registering || registered || !REGISTRY_ADDRESS} className="flex w-full items-center justify-center gap-2 bg-foreground px-3 py-2 text-[10px] font-bold uppercase tracking-[0.13em] text-background disabled:cursor-not-allowed disabled:opacity-50"><Fingerprint size={12} /> {registered ? "Registered" : registering ? "Registering…" : "Register on 0G"}</button>}</div></article>
}
...
function PolicyCard({ agent, registered, syncing, onSave, onSync }: { agent: Agent; registered: boolean; syncing: boolean; onSave: (agentId: string, maxTransaction: number, dailyBudget: number, active: boolean, protocols: string[]) => boolean; onSync: () => void }) {
  ...
  const save = () => {
    ...
    try {
      normalizeProtocolAddresses(protocols)
    } catch (validationError) {
      setError(validationError instanceof Error ? validationError.message : "Use valid EVM protocol addresses.")
      return
    }
    if (!protocols.length) {
      setError("Add at least one protocol address.")
      return
    }
      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2">
        {editing ? (
          <>
            ...
            <label className="sm:col-span-2"><span className="field-label">Approved protocol addresses, comma separated</span><input className="field-input" value={draft.protocols} onChange={(event) => setDraft((current) => ({ ...current, protocols: event.target.value }))} placeholder="0x1234…, 0xabcd…" spellCheck={false} /></label>
          ...
          <>
            ...
            <div className="sm:col-span-2"><p className="field-label">Approved protocol addresses</p><div className="mt-2 flex flex-wrap gap-2">{agent.protocols.length ? agent.protocols.map((protocol) => <span key={protocol} title={protocol} className="border border-foreground px-2 py-1 text-[10px] uppercase tracking-[0.1em]">{shortAddress(protocol)}</span>) : <span className="text-xs text-muted-foreground">No protocol addresses configured.</span>}</div></div>
            <div className="grid gap-2 sm:col-span-2 sm:grid-cols-2"><button onClick={() => setEditing(true)} className="border-2 border-foreground px-4 py-2 text-[10px] font-bold uppercase tracking-[0.13em] hover:bg-foreground hover:text-background">Edit policy</button><button onClick={onSync} disabled={!registered || syncing || agent.onchainPolicySynced} className="bg-foreground px-4 py-2 text-[10px] font-bold uppercase tracking-[0.13em] text-background disabled:cursor-not-allowed disabled:opacity-50">{syncing ? "Syncing…" : !registered ? "Register before sync" : agent.onchainPolicySynced ? "Synced on 0G" : "Sync to 0G"}</button></div>
          </>
        )}
      </div>
    </article>
...
function ActivityRow({ item, detailed = false }: { item: ActivityItem; detailed?: boolean }) {
  return <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><div className={`mt-1 grid h-7 w-7 shrink-0 place-items-center border ${item.decision === "BLOCKED" ? "border-[#b91c1c] text-[#b91c1c]" : item.decision === "REVIEW" ? "border-[#a16207] text-[#a16207]" : "border-[#3f6212] text-[#3f6212]"}`}>{item.decision === "BLOCKED" ? <X size={13} /> : item.decision === "REVIEW" ? <CircleAlert size={13} /> : <Check size={13} />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-bold uppercase tracking-[0.08em]">{item.action} {item.amount > 0 ? `${item.amount} ${item.asset}` : item.asset}</p><DecisionBadge decision={item.decision} />{item.txStatus && item.txStatus !== "LOCAL" && <span className="border border-border px-2 py-1 text-[9px] font-bold tracking-[0.1em] text-muted-foreground">{item.txStatus.replaceAll("_", " ")}</span>}</div><p className="mt-1 truncate text-xs text-muted-foreground">{item.agent} · {item.destination}</p>{detailed && <p className="mt-2 text-[11px] text-muted-foreground">{item.reasons.join(" · ")}</p>}</div></div><div className="flex shrink-0 items-center gap-4 text-[10px] uppercase tracking-[0.12em] text-muted-foreground sm:text-right"><span>{formatActivityTime(item.createdAt)}</span><span className="font-bold text-foreground">RISK {item.risk}</span>{item.decisionTxHash && /^0x[0-9a-fA-F]{64}$/.test(item.decisionTxHash) && <a href={`${TESTNET.explorer}/tx/${item.decisionTxHash}`} target="_blank" rel="noreferrer" className="hover:text-foreground" title="Onchain decision"><ShieldCheck size={12} /></a>}{item.txHash && /^0x[0-9a-fA-F]{64}$/.test(item.txHash) && <a href={`${TESTNET.explorer}/tx/${item.txHash}`} target="_blank" rel="noreferrer" className="hover:text-foreground" title="Guarded transaction"><ExternalLink size={12} /></a>}</div></div>
}
